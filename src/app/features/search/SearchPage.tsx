...
import { RateLimitError } from '../../infra/http/errors';
        ...
        <div className="mt-4">
          {isTooShort ? (
            ...
            <ApiErrorState error={search.error} onRetry={() => void search.refetch()}>
              {/* Com a Busca bloqueada, o link direto usa o outro limite de requisições. */}
              {search.error instanceof RateLimitError && isValidUsername(term) && (
                <Link className="btn btn-outline-primary" to={userPath(term)}>
                  Abrir o perfil de {term}
                </Link>
              )}
            </ApiErrorState>
